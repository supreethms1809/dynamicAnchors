...
def main():
    ...
    if not args.load_checkpoint:
        ...
        logger.info(f"\nTo score extracted rules, run:")
        logger.info(f"  python -m revision.evaluate --rules_file {checkpoint_path}/inference/extracted_rules.json --dataset {args.dataset}")
        logger.info(f"\nThis will:")
        ...
        logger.info(f"  4. Score stored boxes with revision.evaluate")
    else:
        logger.info(f"\nTo extract rules, run:")
        logger.info(f"  python inference.py --experiment_dir {checkpoint_path} --dataset {args.dataset}")
    logger.info(f"{'='*80}")
