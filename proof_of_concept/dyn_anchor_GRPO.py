...
class AnchorEnv:
    ...
    - Reward: Vectorized reward [precision_gain, coverage_gain, -overlap_penalty, -drift_penalty, -js_penalty]
              Also returns scalar reward in info dict for backward compatibility:
              precision_gain * alpha + coverage_gain * beta - overlap_penalty - drift_penalty - js_penalty
    """
    ...
    def step(self, action: int):
        ...
        # Vectorized reward: [precision_gain, coverage_gain, -overlap_penalty, -drift_penalty, -js_penalty]
        ...
        # Scalar reward for backward compatibility (computed from vectorized components)
        reward_scalar = self.alpha * precision_gain + self.beta * coverage_gain - overlap_penalty - drift_penalty - js_penalty
        ...
        info = {"precision": precision, "coverage": coverage, "drift": drift, "js_penalty": js_penalty, "coverage_clipped": coverage_clipped, "reward_scalar": reward_scalar, **details}
...
def train_dynamic_anchors(
    ...
    """
    Train dynamic anchors using GRPO (Group Relative Policy Optimization) with vectorized rewards.
    
    This version uses vectorized rewards instead of scalar rewards. The reward vector contains:
    [precision_gain, coverage_gain, -overlap_penalty, -drift_penalty, -js_penalty]
    
    The scalar reward for PPO is computed as: alpha * precision_gain + beta * coverage_gain 
    - overlap_penalty - drift_penalty - js_penalty
    ...
    reward_vectors_per_episode = []  # Track vectorized rewards per episode
    ...
    for ep in range(episodes):
        ...
        all_actions = []
        all_old_log_probs = []
        all_rewards = []  # Scalar rewards for PPO
        all_reward_vectors = []  # Vectorized rewards for analysis
        all_values = []
        all_dones = []
        all_entropies = []
        gamma = 0.99
        ...
        for cls, env in envs.items():
            ...
            class_actions = []
            class_old_log_probs = []
            class_rewards = []  # Scalar rewards
            class_reward_vectors = []  # Vectorized rewards
            class_values = []
            class_dones = []
            class_entropies = []
            ...
            for t in range(steps_per_episode):
                ...
                next_state, reward_vector, done, info = env.step(int(action.item()))
                
                # Extract scalar reward from info (computed in step method)
                reward_scalar = info.get("reward_scalar", 0.0)
                ...
                class_rewards.append(float(reward_scalar))
                class_reward_vectors.append(reward_vector.copy())
                ...
                # Accumulate reward components from vectorized reward
                comp_sums["prec_gain"] += float(reward_vector[0])
                comp_sums["cov_gain"] += float(reward_vector[1])
                comp_sums["overlap_pen"] += float(-reward_vector[2])
                comp_sums["drift_pen"] += float(-reward_vector[3])
                comp_sums["js_pen"] += float(-reward_vector[4])
                state = next_state
                if done:
                    break
            ...
            all_reward_vectors.extend(class_reward_vectors)
        ...
        # Store vectorized rewards for this episode
        reward_vectors_per_episode.append([rv.copy() for rv in all_reward_vectors])
    ...
    return {
        ...
        "value_fn": value_fn,  # PyTorch model - should be saved separately
        ...
        "reward_vectors_per_episode": reward_vectors_per_episode,  # Vectorized rewards for analysis
...
def load_trained_models(
    ...
    policy = PolicyNet(state_dim, 1000).to(device)  # Temporary action_dim
    policy.load_state_dict(torch.load(policy_file, map_location=device))
    # Infer action_dim from loaded weights
    action_dim = policy.net[-1].out_features
    policy = PolicyNet(state_dim, action_dim).to(device)
    policy.load_state_dict(torch.load(policy_file, map_location=device))
    policy.eval()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run dynamic anchor training with GRPO (vectorized rewards).")
    ...
    if policy_model is not None:
        ...
        torch.save(policy_model.state_dict(), policy_file)
        print(f"Policy saved to {policy_file}")
    if value_model is not None:
        value_file = f'value_fn_{model_prefix}.pth'
        torch.save(value_model.state_dict(), value_file)
