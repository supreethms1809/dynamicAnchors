...
def summarize_rules_from_json(rules_data: Dict) -> Dict:
    ...
    for class_key, class_data in per_class_results.items():
        ...
        # Count features in class-level rules
        ...
        class_summary = {
            ...
            "class_level_precision": float(class_level_precision),
            "class_level_coverage": float(class_level_coverage),
            # Legacy names for backward compatibility
            "class_based_precision": float(class_level_precision),
            "class_based_coverage": float(class_level_coverage),
            "class_level_n_total_rules": len(class_level_all_rules),
            "class_level_n_unique_rules": len(class_level_unique_rules),
            "class_level_unique_rules": class_level_unique_rules,
            # Legacy names
            "class_based_n_total_rules": len(class_level_all_rules),
            "class_based_n_unique_rules": len(class_level_unique_rules),
            "class_based_unique_rules": class_level_unique_rules,
        }
        ...
        if class_level_precision > 0 or class_level_coverage > 0:
            all_precisions.append(class_level_precision)
            all_coverages.append(class_level_coverage)
            all_rule_counts.append(len(class_level_all_rules))
            all_unique_rule_counts.append(len(class_level_unique_rules))
...
def plot_precision_coverage_tradeoff(summary: Dict, output_dir: str, dataset_name: str = ""):
    ...
    ax2.set_title(f'{title_prefix}: Class Union Precision vs Coverage Trade-off', fontsize=14, fontweight='bold')
...
def generate_summary_report(summary: Dict, test_results: Optional[Dict] = None, output_file: str = "summary_report.txt"):
    ...
    with open(output_file, 'w') as f:
        ...
        f.write("(Global averages removed - see per-class details below)\n")
