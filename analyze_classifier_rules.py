...
class ClassifierRulesAnalyzer:
    ...
    def generate_class_report(self, target_class: int, class_name: Optional[str] = None) -> str:
        ...
        report_lines.append("-" * 80)
        report_lines.append(f"Instance-Level Precision: {rules_info['instance_precision']:.4f}")
        report_lines.append(f"  → Average precision across individual instance-based rules")
        report_lines.append("")
        report_lines.append(f"Instance-Level Coverage: {rules_info['instance_coverage']:.4f}")
        report_lines.append(f"  → Average coverage across individual instance-based rules")
        report_lines.append("")
        report_lines.append(f"Class Union Precision: {rules_info['class_precision']:.4f}")
        ...
        report_lines.append("")
        report_lines.append(f"Class Union Coverage: {rules_info['class_coverage']:.4f}")
